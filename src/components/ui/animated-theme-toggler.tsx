...
export interface AnimatedThemeTogglerProps {
  sound?: boolean;
  /** Override icon color (e.g. "rgba(255,255,255,0.82)" on dark hero bg) */
  iconColor?: string;
}
...
export function AnimatedThemeToggler({
  ...
  // next-themes: resolvedTheme handles "system" → actual resolved value
  ...
  // Before mount: render placeholder matching button size to avoid layout shift
  if (!mounted) {
  ...
  // Default ink color follows dark/light via CSS var; override on hero
  const color = iconColor ?? "var(--at-ink)";
