      ...
      <div className="min-h-screen pt-24 pb-16 relative">
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8 relative z-10">
          ...
          {searchQuery === "" && activeCategory === "all" && currentPage === 1 && (
            <FeaturedInsightsHub language="en" />
          )}
                ...
                <div className="aspect-[4/3] rounded-xl overflow-hidden relative">
                  ...
                  {featuredPost.hasDashboard && (
                    <div className="absolute top-4 left-4 flex items-center gap-1.5 bg-primary/90 backdrop-blur-sm text-primary-foreground px-3 py-1.5 rounded-full text-sm font-medium">
                      <BarChart3 className="w-4 h-4" />Interactive Dashboard
                    </div>
                  )}
                </div>
                <div className="flex flex-col justify-center">
                  <span className="text-primary text-sm font-medium mb-4">{featuredPost.category}</span>
                  <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4 group-hover:text-primary transition-colors leading-tight">{featuredPost.title}</h2>
                  <p className="text-muted-foreground mb-6 line-clamp-3 leading-relaxed">{featuredPost.excerpt}</p>
          ...
          >
            {displayPosts.length > 0 ? (
              <>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 mb-16">
                  {displayPosts.map((post, index) => (
                    <Link key={post.id} to={`/blog/${post.slug}`} className="group" style={{ animationDelay: `${index * 100}ms` }}>
                        ...
                        <div className="aspect-[3/2] rounded-lg overflow-hidden mb-5 relative">
                          ...
                          {post.hasDashboard && (
                            <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-primary/90 backdrop-blur-sm text-primary-foreground px-2.5 py-1 rounded-full text-xs font-medium">
                              <BarChart3 className="w-3 h-3" />Interactive
                            </div>
                          )}
                        </div>
                        <span className="text-primary/70 text-sm font-medium mb-2">{post.category}</span>
                        <h3 className="text-xl font-semibold text-foreground mb-3 group-hover:text-primary transition-colors line-clamp-2 leading-snug">{post.title}</h3>
              ...
              <div className="text-center py-20">
                <h3 className="text-xl font-semibold text-foreground mb-2">{bt.noResults}</h3>
                <p className="text-muted-foreground mb-6">{bt.noResultsDescription}</p>
                <Button onClick={handleClearFilters} variant="outline" className="border-border">{bt.clearFilters}</Button>
              </div>
            )}
          </div>
        </div>
