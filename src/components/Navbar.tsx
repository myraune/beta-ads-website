  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      <nav className="lg:hidden">
        <div className="flex items-center justify-between px-4 h-[56px] bg-background/80 backdrop-blur-xl shadow-md shadow-black/10">
          <Link to="/">
          ...
          <div className="flex items-center gap-2">
            {mounted && (
              <Button
                ...
                size="sm"
                onClick={toggleTheme}
                className="h-9 w-9 rounded-lg text-foreground transition-colors duration-150 p-0"
                aria-label={theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}
              >
            ...
            <button
              ...
              className="p-2 text-foreground rounded-lg transition-colors duration-150"
        ...
        <div className={`overflow-hidden transition-[max-height,opacity] duration-300 ease-out ${isOpen ? "max-h-[400px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="px-4 py-4 bg-background/95 backdrop-blur-xl shadow-lg shadow-black/10 space-y-2">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                to={link.href}
                className={`block px-4 py-3 text-sm font-light tracking-wide rounded-lg transition-colors duration-150 ${
                  location.pathname === link.href
                    ? "text-primary"
                    : "text-muted-foreground hover:text-foreground"
                }`}
            ...
            <div className="pt-3 border-t border-border">
              <Link to="/demo" className="block">
                <Button
                  size="sm"
                  className="w-full bg-primary text-primary-foreground hover:bg-primary/90 text-sm font-light tracking-wide h-10 rounded-lg shadow-md shadow-primary/20 transition-colors duration-150"
                >
                  Book a Demo
