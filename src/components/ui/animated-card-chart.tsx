...
export function ChartCardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
...
export function ChartCardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
...
export function ChartCardDescription({ className, ...props }: CardDescriptionProps) {
  return (
    <p
      className={cn(
...
export function ChartCardVisual({ className, ...props }: CardProps) {
  return (
    <div
...
const EllipseGradient: React.FC<{ color: string }> = ({ color }) => {
  return (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
      ...
      >
        <rect width="356" height="180" fill="url(#paint0_radial_chart)" />
        <defs>
          <radialGradient
            id="paint0_radial_chart"
            cx="0"
            cy="0"
            r="1"
  ...
  return (
    ...
    >
      <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
        <div className="ease-[cubic-bezier(0.6,0,1,1)] rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
  ...
  return (
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[6] flex translate-y-full items-center justify-center opacity-0 transition-all duration-500 group-hover/animated-card:translate-y-0 group-hover/animated-card:opacity-100">
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
      >
        <rect width="356" height="180" fill="url(#paint0_linear_chart)" />
        <defs>
          <linearGradient
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[8] flex h-[180px] w-[356px] items-center justify-center text-[#5adbb5]/30 transition-transform duration-500 group-hover/animated-card:scale-150 dark:text-[#5adbb5]/25">
